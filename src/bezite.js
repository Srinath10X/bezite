...
const createStatelessElement = (object) =>
  isHTMLelement(object) || instanceOf(object, Text) ? object : document.createTextNode(object);
const createStatefullElement = (object) => {
  const element = createStatelessElement(object.value);
  object.effect((newValue) =>
    isHTMLelement(newValue) ? element.replaceWith(newValue) : (element.nodeValue = newValue),
  );
  return element;
};
...
const processedChildren = (children) =>
  [children]
    .flat(Infinity)
    .map((child) => (isReactive(child) ? createStatefullElement(child) : createStatelessElement(child)));
...
export const createElement = (tag, attributes, ...children) => {
  ...
  for (let key in attributes) {
    ...
    key.substring(0, 2) == "on" ? element.addEventListener(key.slice(2), value) : setAttributes(element, key, value);
  }
